...
passport.use(
	new LocalStrategy({ usernameField: "usernameOrEmail" }, async (usernameOrEmail, password, next) => {
		// Check if user typed in their email or username
		let searchParams = emailRegex.test(usernameOrEmail) ? { email: usernameOrEmail } : { username: usernameOrEmail };
